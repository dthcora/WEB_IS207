...
if (isset($_GET['slider_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt_delete->execute()) {
            header("Location: list-slider.php?message=Slider deleted successfully");
        } else {
            header("Location: list-slider.php?error=Failed to delete slider");
        }
    } else {
        header("Location: list-slider.php?error=Slider not found");
    }
} else {
    header("Location: list-slider.php?error=Invalid slider ID");
}
?>
